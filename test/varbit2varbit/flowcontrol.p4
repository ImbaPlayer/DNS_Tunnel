...
struct my_ingress_headers_t {
    ...
    // single_label_t[10] label_list;
    single_label_t label_1;
    dns_q_label_len_t q_label_len_2;
    single_label_t label_2;
    bit_test_t bit_test;
}
   ...
   state parse_port_metadata {
    ...
    state parse_dns_query {
        ...
        transition select(counter.is_zero()) {
            ...
            false: parse_single_label;
        }
    }
    // state parse_single_label {
    //     // pkt.extract(hdr.label_list.next, (bit<32>)hdr.q_label_len.label_len);
    //     pkt.extract(hdr.label_list.next);
    //     transition parse_dns_query;
    // }

    state parse_single_label {
        // pkt.extract(hdr.label_1, (bit<32>)((bit<6>)hdr.q_label_len.label_len << 3));
        pkt.extract(hdr.label_1, ((bit<32>)hdr.ipv4.ihl - 5) << 5);
        // pkt.extract(hdr.label_1, 32w8);
        transition parse_dns_query_2;
    }

    state parse_dns_query_2 {
        pkt.extract(hdr.q_label_len_2);
        counter.set(hdr.q_label_len_2.label_len);
        // counter.set(8w4);
        transition select(counter.is_zero()) {
            true: finish_parse_domain;
            false: parse_single_label_2;
        }
    }
    state parse_single_label_2 {
        // pkt.extract(hdr.label_2, 32w8);
        transition finish_parse_domain;
    }
