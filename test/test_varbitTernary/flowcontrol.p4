...
header dns_label_part_t {
    varbit<512> label_part;
}
...
struct my_ingress_headers_t {
    ...
    dns_label_part_t dns_label_part_1;
    dns_label_part_t dns_label_part_2;
    // dns_label_part_t dns_label_part_3;
    // dns_label_part_t dns_label_part_4;
}
   ...
   state dispatch_on_protocol {
    ...
    state parse_dns_query {
        ...
        transition select(hdr.q_label_len.label_len) {
            0: finish_parse_domain;
            default: parse_label_1;
        }
        
    }

    state parse_label_1 {
        // pkt.extract(hdr.dns_label_part_1, ((bit<32>)hdr.q_label_len.label_len) << 3);
        pkt.extract(hdr.dns_label_part_1, (bit<32>)(((bit<6>)hdr.q_label_len.label_len) * 8));
        // pkt.extract(hdr.dns_label_part_1, ((bit<32>)hdr.ipv4.ihl - 5) << 5);
        transition parse_dns_part_2;
    }

    state parse_dns_part_2 {
        pkt.extract(hdr.q_label_len);
        transition select(hdr.q_label_len.label_len) {
            0: finish_parse_domain;
            default: parse_label_2;
        }
    }

    state parse_label_2 {
        pkt.extract(hdr.dns_label_part_2, (bit<32>)(((bit<6>)hdr.q_label_len.label_len) * 8));
        // transition parse_dns_part_2;
        transition accept;
    }
